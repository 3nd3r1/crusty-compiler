...
pub fn assemble_and_get_executable(code: &str, workdir: Option<&Path>) -> Result<Vec<u8>, String> {
    let workdir = workdir.unwrap_or_else(|| Path::new("./output"));
    ...
    fs::write(&stdlib_asm, STDLIB_ASM_CODE)
        .map_err(|e| format!("Failed to write stdlib.s: {}", e))?;
    fs::write(&program_asm, code).map_err(|e| format!("Failed to write program.s: {}", e))?;

    let stdlib_as_status = std::process::Command::new("as")
        .args(&[
            "-q",
            "-o",
            stdlib_obj.to_str().unwrap(),
            stdlib_asm.to_str().unwrap(),
        ])
        .status()
        .map_err(|e| format!("Failed to run as: {}", e))?;

    if !stdlib_as_status.success() {
        return Err(format!("as failed with status: {}", stdlib_as_status));
    }

    let program_as_status = std::process::Command::new("as")
        .args(&[
            "-q",
            "-o",
            program_obj.to_str().unwrap(),
            program_asm.to_str().unwrap(),
        ])
        .status()
        .map_err(|e| format!("Failed to run as on program: {}", e))?;

    if !program_as_status.success() {
        return Err(format!(
            "as failed on program with status: {}",
            program_as_status
        ));
    }
    ...
    if !ld_status.success() {
        return Err(format!("ld failed with status: {}", ld_status));
    }

    let executable_bytes =
        fs::read(&output_file).map_err(|e| format!("Failed to read output executable: {}", e))?;

    Ok(executable_bytes)
}

const STDLIB_ASM_CODE: &str = r#"
    .global _start
    .global print_int
    .global print_bool
    .global read_int
